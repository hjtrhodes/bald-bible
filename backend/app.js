...
const { devDbPassword } = require('./config'); // import the password property of the object exported from config.js

let password = process.env.MONGO_PASSWORD || devDbPassword; 
...
app.use('/api/images', imageRoutes); // call the use method, which adds a middleware function to the middleware stack, to handle requests to the /api/stuff endpoint
